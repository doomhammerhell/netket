...
@struct.dataclass
class CombinedRule(MetropolisRule):
    ...
    def __post_init__(self):
        if not isinstance(self.hilbert, TensorHilbert):
            raise TypeError(
        ...
        if len(self.hilbert.subspaces) != len(self.rules):
            raise ValueError(
                "Length mismatch between the rules and the hilbert space: Hilbert "
                f"has {len(self.hilbert.subspaces)} subpsaces, but you specified {len(self.rules)}."
            )
    ...
    def __repr__(self):
        return "CombinedRule(hilbert={self.hilbert}, rules={self.rules})"
